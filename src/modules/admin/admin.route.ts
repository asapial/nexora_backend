...
import { approvePriceSchema, createUsersByEmailSchema, rejectNoteSchema, revenuePercentSchema } from "../../validation/requestSchemas";
...
router.post("/courses/:id/reject", checkAuth(Role.ADMIN), validateRequest(rejectNoteSchema), adminController.rejectCourse);
...
router.patch("/courses/:id/revenue-percent", checkAuth(Role.ADMIN), validateRequest(revenuePercentSchema), adminController.setRevenuePercent);
...
router.post("/missions/:id/reject", checkAuth(Role.ADMIN), validateRequest(rejectNoteSchema), adminController.rejectMission);

// Price requests
router.get("/price-requests", checkAuth(Role.ADMIN), adminController.getPendingPriceRequests);
router.post("/price-requests/:id/approve", checkAuth(Role.ADMIN), validateRequest(approvePriceSchema), adminController.approvePriceRequest);
router.post("/price-requests/:id/reject", checkAuth(Role.ADMIN), validateRequest(rejectNoteSchema), adminController.rejectPriceRequest);
