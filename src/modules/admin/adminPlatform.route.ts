...
import { emailTemplateCreateSchema, emailTemplateUpdateSchema, globalAnnouncementSchema, manualEnrollmentSchema, warningSchema } from "../../validation/requestSchemas";
...
router.post("/announcements", checkAuth(Role.ADMIN), validateRequest(globalAnnouncementSchema), adminPlatformController.createGlobalAnnouncement);
...
router.post("/moderation/warn/:userId", checkAuth(Role.ADMIN), validateRequest(warningSchema), adminPlatformController.warnUser);
...
router.post("/enroll", checkAuth(Role.ADMIN), validateRequest(manualEnrollmentSchema), adminPlatformController.manualEnroll);
router.post("/unenroll", checkAuth(Role.ADMIN), validateRequest(manualEnrollmentSchema), adminPlatformController.manualUnenroll);

// Email Templates
router.get("/email-templates", checkAuth(Role.ADMIN), adminPlatformController.getEmailTemplates);
router.post("/email-templates", checkAuth(Role.ADMIN), validateRequest(emailTemplateCreateSchema), adminPlatformController.createEmailTemplate);
router.patch("/email-templates/:id", checkAuth(Role.ADMIN), validateRequest(emailTemplateUpdateSchema), adminPlatformController.updateEmailTemplate);
